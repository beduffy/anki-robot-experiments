...
def calculate_lstm_input_size_after_4_conv_layers(frame_dim, stride=2, kernel_size=3, padding=1,
    ...
    width = (width - kernel_size + 2 * padding) // stride + 1
    if num_layers != 3:  ## todo doesn't work for GA
        width = (width - kernel_size + 2 * padding) // stride + 1
...
class A3C_LSTM_GA(torch.nn.Module):
    ...
    def __init__(self, num_input_channels, num_outputs, frame_dim, num_words):
        ...
        self.lstm_cell_size = calculate_lstm_input_size_after_4_conv_layers(frame_dim, num_layers=3)

        # Instruction Processing
        self.gru_hidden_size = 256
        self.embedding = nn.Embedding(num_words, 32)
        ...
        # A3C-LSTM layers
        # self.linear = nn.Linear(self.lstm_cell_size, 256) # todo maybe 150, 150 res
        self.linear = nn.Linear(64 * 3 * 3, 256) # todo maybe 150, 150 res
    ...
    def forward(self, inputs):
        ...
        for i in range(input_inst.data.size(1)):
            ...
            word_embedding = self.embedding(input_inst[0, i].long()).unsqueeze(0).unsqueeze(0)  # crazy todo
        ...
        x_attention = x_attention.expand(1, 64, 3, 3) # 80x80 input
